...
이 스킬은 이미 CI/CD 파일에 릴리스 동작이 표현된 Android 레포를 대상으로 합니다.
...
- Android application 모듈이 여러 개인 경우
- 커스텀 Gradle 플러그인이 버전 값을 생성하는 경우
- version catalog 또는 convention plugin 기반 버전 관리
- `1.0`, `1.0.0-beta`, `2026.05`처럼 semver `X.Y.Z`가 아닌 `versionName`
- 수동 실행, 예약 실행, 또는 태그 기반 여부가 명확하지 않은 릴리스 트리거
...
이 프로젝트는 독립 프로젝트이며 Google, Android, GitHub, GitLab, Jenkins 또는 여기에 언급된 CI/CD 벤더와 제휴, 보증, 후원 관계가 없습니다. 제품명과 로고는 호환 워크플로를 설명하기 위한 목적으로만 사용합니다.
